...
(function() {
	...
	window._addStat = function(cP, cO) {
		...
		var cQ = function(cR) {
			...
			setTimeout(function() {
				_addStat._img.src ="style/"+cc.imgPath + "blank.gif?" + cR.src
			}, 50)
		};
		var T = function() {
	...
	if (a8.browser.firefox) {
		...
		if (a8.browser.chrome || a8.browser.safari) {
			a8.extend(cc, {
				...
				defaultCursor : "url(style/" + cc.imgPath
						+ "openhand.cur) 8 8,default",
				draggingCursor : "url(style/" + cc.imgPath
						+ "closedhand.cur) 8 8,move"
			})
		} else {
			a8.extend(cc, {
				distCursor : "url(style/"+ cc.imgPath + "ruler.cur),crosshair",
				defaultCursor : "url(style/" + cc.imgPath + "openhand.cur),default",
				draggingCursor : "url(style/" + cc.imgPath + "closedhand.cur),move"
			})
		}
	}
	function av(cM, cL, T) {
	...
	a8.object.extend(cA, {
		...
		preLoaded : {},
		Config : {
			_baseUrl : "style/js", //x + "getmodules?v=1.3",                          //******* �Ķ���Դ��map,oppc,tile,control��·�� *******
			_timeout : 5000
		},
		delayFlag : false,
		Module : {
	...
	bz.prototype.openInfoWindow = function(cN, T) {
		...
		var cL = this.temp;
		if (!cL.marker) {
			var cM = new M("style/"+cc.imgPath + "blank.gif", {
				width : 1,
				height : 1
			});
			cL.marker = new ad(T, {
	...
	function bU(cR, T, cO, cL, cN) {
		...
		cQ.left = cO[0] + "px";
		cQ.top = cO[1] + "px";
		cQ.border = "#ccc solid 1px";
		//link.href = "/jiaotongTing/DrawRoute/findtile?tile_x="+cL[0]+"&tile_y="+cL[1];
		link.addEventListener("click",function(){
			$(".xcConfirm").attr("id",cL[0]+","+cL[1]);
            $("#points").html('');
			$('input[name="imgurl"]').val("maptile/17/"+cL[0]+"/"+cL[1]+".png");
            $('input[name="tile_x"]').val(cL[0]);
            $('input[name="tile_y"]').val(cL[1]);
            var dire = $('input[name="directions"]').val();
            console.log(dire);
            $.ajax({
                cache: false,
                type: "GET",
                url:"drawroute/findpoint?x="+cL[0]+"&y="+cL[1]+"&d="+dire,
                async: true,
                ...
                success: function(data) {
                    ...
                    $('input[name="tileId"]').val(jso['id']);
                    $(".tt").html("当前瓦片编号 "+jso['id']);
                    for(var p in jso['point']){//遍历json对象的每个key/value对,p为key
                        var innerHtml = "<li class='dd-item'  data-id='"+jso['point'][p][0]+","+jso['point'][p][1]+"'><div class='dd-handle'>"+jso['point'][p][0]+","+jso['point'][p][1]+"</div><i class='js-remove' onclick='removePoint("+jso['point'][p][0]+","+jso['point'][p][1]+")'>✖</i></li>";
		...
		this.img.onload = function(cY) {
			...
			if (cP.img && !y(cP.img)) {
				if (cN.tilesDiv) {
					cN.tilesDiv.appendChild(cP.a).appendChild(cP.img);
	...
	bU.prototype._load = function() {
		if (a8.browser.ie > 0 && a8.browser.ie <= 6 && this._transparentPng) {
			this.img.src = "style/"+cc.imgPath + "blank.gif"
		} else {
